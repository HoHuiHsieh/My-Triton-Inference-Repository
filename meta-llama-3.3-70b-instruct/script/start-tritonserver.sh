...
DEPLOY=false
CONTAINER_NAME="triton-llama-3.3-70b-instruct"
WORKSPACE=/workspace
...
if [ "$DEPLOY" = true ]; then
    CMD="python3 ./src/launch_triton_server.py --model_repo ${WORKSPACE}/repository"
else
    CMD="bash"
fi
...
docker run -itd --gpus all \
    ...
    -p 8002:8002 \
    -w ${WORKSPACE} \
    nvcr.io/nvidia/tritonserver:25.03-trtllm-python-py3 ${CMD}
