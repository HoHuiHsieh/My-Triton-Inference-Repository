#!/usr/bin/env python
"""
Test script for llama-embed-nemotron-8b model on Triton Inference Server.
...
def test_similarity_search(triton_client, model_name, query, documents):
    ...
    for i in range(len(documents)):
        doc_emb = doc_embeddings[i] if doc_embeddings.ndim == 2 else doc_embeddings
...
def main():
    ...
    print("="*80)
    print("NVIDIA Llama Embed Nemotron 8B - Triton Inference Server Tests")
    print("="*80)
    print(f"Triton Server URL: {triton_url}")
    print(f"Model Name: {model_name}")
    
    # Test data - matching the example from the model
